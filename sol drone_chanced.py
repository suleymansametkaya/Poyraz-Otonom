...
results_list = []  # Sonuçları saklamak için liste
counter = 0
...
while True:
    ...
    if counter == 10:
        counter = 0
        try:
            results_list.append(result)  # Sonucu listeye ekle
        except Exception as e:
            print(f"Sonuç ekleme hatası: {e}")

# Döngü bittikten sonra verileri dosyaya yaz
with open("data.txt", "w") as file:
    for result in results_list:
        try:
            yaz(file, result)  # Sonuçları dosyaya yaz
        except Exception as e:
